...
static void mkl_spmm(const int argc, const char *argv[], const char *file, int thread_num, sparse_index_base_t *ret_index, MKL_INT *ret_rows, MKL_INT *ret_cols, MKL_INT **ret_rows_start, MKL_INT **ret_rows_end, MKL_INT **ret_col_index, MKL_Complex8 **ret_values)
{
    ...
    mkl_sparse_c_create_coo(&cooB, SPARSE_INDEX_BASE_ZERO, m, k, nnz, row_index, col_index, values);
    ...
    mkl_call_exit(mkl_sparse_c_export_bsr(result, ret_index, &mkl_block_layout, ret_rows, ret_cols, &mkl_block_size, ret_rows_start, ret_rows_end, ret_col_index, ret_values), "mkl_sparse_c_export_bsr");
...
static void alpha_spmm(const int argc, const char *argv[], const char *file, int thread_num, alphasparse_index_base_t *ret_index, ALPHA_INT *ret_rows, ALPHA_INT *ret_cols, ALPHA_INT **ret_rows_start, ALPHA_INT **ret_rows_end, ALPHA_INT **ret_col_index, ALPHA_Complex8 **ret_values, int flag)
{
    ALPHA_INT m, k, nnz;
    ...
    alphasparse_c_create_coo(&cooA, ALPHA_SPARSE_INDEX_BASE_ZERO, m, k, nnz, row_index, col_index, values);
    ...
    alphasparse_c_create_coo(&cooB, ALPHA_SPARSE_INDEX_BASE_ZERO, m, k, nnz, row_index, col_index, values);
    ...
    alpha_call_exit(alphasparse_c_export_bsr(result, ret_index, &alpha_block_layout, ret_rows, ret_cols, &alpha_block_size, ret_rows_start, ret_rows_end, ret_col_index, ret_values), "alphasparse_c_export_bsr");
...
int main(int argc,const char *argv[])
{
    ...
    */
    ...
    if (check)
    {
        ...
        status = check_s((float *)mkl_values, 2*mkl_nnz*BLOCK_SIZE*BLOCK_SIZE, (float *)alpha_values, 2*alpha_nnz*BLOCK_SIZE*BLOCK_SIZE);
    }
