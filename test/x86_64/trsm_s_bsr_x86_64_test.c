...
static void mkl_trsm_bsr(const int argc, const char *argv[], const char *file, int thread_num, const float alpha, float **ret, size_t *size, sparse_layout_t b_layout)
{
    MKL_INT m, k, nnz;
    MKL_INT *row_index, *col_index;
    float *values;
    ...
    if(b_layout == SPARSE_LAYOUT_COLUMN_MAJOR)
    {
        ...
        mkl_sparse_s_create_coo(&cooA, SPARSE_INDEX_BASE_ONE, m, k, nnz, row_index, col_index, values);
    }
    else
    {
        mkl_sparse_s_create_coo(&cooA, SPARSE_INDEX_BASE_ZERO, m, k, nnz, row_index, col_index, values);
    ...
    mkl_call_noexit(mkl_sparse_s_trsm(transA, alpha, bsrA, descr, layout, x, columns, ldx, y, ldy), "mkl_sparse_s_trsm");
    // mkl_sparse_s_trsm(transA, alpha, bsrA, descr, layout, x, columns, ldx, y, ldy);
    ldy=ldx;
    ldx=ldy;
...
static void alpha_trsm(const int argc, const char *argv[], const char *file, int thread_num, const float alpha, float **ret, size_t *size, alphasparse_layout_t b_layout)
{
    ALPHA_INT m, k, nnz;
    ...
    alpha_timer_t timer;
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_s_trsm_plain(transA, alpha, bsrA, descr, layout, x, columns, ldx, y, ldy), "alphasparse_s_trsm_plain");
    alpha_timing_end(&timer);
