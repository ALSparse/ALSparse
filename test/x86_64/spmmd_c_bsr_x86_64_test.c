...
static void mkl_spmmd(const int argc, const char *argv[], const char *file, int thread_num, MKL_Complex8 **ret, size_t *ret_size)
{
	...
	for(int i = 0;i<iter;i++){    
		alpha_clear_cache();    
		alpha_timing_start(&timer);    
		mkl_call_exit(mkl_sparse_c_spmmd(transA, bsrA, bsrB, layout, C, ldc), "mkl_sparse_c_spmmd");    
		alpha_timing_end(&timer);    
		total_time += alpha_timing_elapsed_time(&timer);    
	}    
	printf("iter is %d, %s avg time : %lf[sec]\n",iter,"mkl_sparse_c_spmmd",total_time/iter);
    //alpha_timing_end(&timer);
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file, int thread_num, ALPHA_Complex8 **ret, size_t *ret_size, alphasparse_layout_t block_layout)
{
	...
	for(int i = 0;i<iter;i++){    
		alpha_clear_cache();    
		alpha_timing_start(&timer);    
		alpha_call_exit(alphasparse_c_spmmd_plain(transA, bsrA, bsrB, layout, C, ldc), "alphasparse_c_spmmd");    
		alpha_timing_end(&timer);    
		total_time += alpha_timing_elapsed_time(&timer);    
	}    
	printf("iter is %d, %s avg time : %lf[sec]\n",iter,"alphasparse_c_spmmd_plain",total_time/iter);;
    //alpha_timing_end(&timer);
