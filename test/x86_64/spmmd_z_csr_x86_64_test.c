...
static void alpha_spmmd(const int argc, const char *argv[], const char *file, int thread_num, ALPHA_Complex16 **ret, size_t *ret_size, ALPHA_INT *ret_ldc)
{
    ...
    alpha_timer_t timer;
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_z_spmmd_plain(transA, csrA, csrB, layout, C, ldc), "alphasparse_z_spmmd");
    alpha_timing_end(&timer);
...
//        status = check_d(double *)mkl_C, 2 * size_mkl_C, (double *)alpha_C, 2 * size_alpha_C);
        status = check_z_l3((ALPHA_Complex16 *)mkl_C, ldc, size_mkl_C, alpha_C, ldc, size_alpha_C, NULL, NULL, 0, alpha_C, ldc, alpha, beta, argc, argv);
        alpha_free(mkl_C);
    }
