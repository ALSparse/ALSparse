...
static void alpha_spmm(const int argc, const char *argv[], const char *file, int thread_num, alphasparse_index_base_t *ret_index, ALPHA_INT *ret_rows, ALPHA_INT *ret_cols, ALPHA_INT **ret_rows_start, ALPHA_INT **ret_rows_end, ALPHA_INT **ret_col_index, float **ret_values)
{
    ALPHA_INT m, k, nnz;
    ...
    alpha_call_exit(alphasparse_spmm(transA, csrA, csrB, &result), "alphasparse_spmm_plain");
...
//        status = check_s(float *)mkl_values, 2*mkl_nnz, (float *)alpha_values, 2*alpha_nnz);
        status = check_s_l3((float * )mkl_values, 0, mkl_nnz, alpha_values, 0, alpha_nnz, alpha_col_index, NULL, 0, alpha_values, 0, alpha, beta, argc, argv);    
    }
