...
static void mkl_spmmd(const int argc, const char *argv[], const char *file, int thread_num, MKL_Complex8 **ret, size_t *ret_size)
{
    ...
    mkl_sparse_c_spmmd(transA, csrA, csrA, layout, C, ldc);
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file, int thread_num, ALPHA_Complex8 **ret, size_t *ret_size)
{
    ALPHA_INT m, k, nnz;
    ...
    alpha_timer_t timer;
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_c_spmmd_plain(transA, csrA, csrA, layout, C, ldc), "alphasparse_c_spmmd");
    alpha_timing_end(&timer);
    printf("%lf,%lf", alpha_timing_elapsed_time(&timer), alpha_timing_gflops(&timer,(double)nnz * k * 2 + m * k));
