...
static void mkl_spmmd(const int argc, const char *argv[], const char *file, int thread_num, float **ret, size_t *ret_size)
{
	...
	for(int i = 0;i<iter;i++){    
		alpha_clear_cache();    
		alpha_timing_start(&timer);    
		mkl_call_exit(mkl_sparse_s_spmmd(transA, bsrA, bsrB, layout, C, ldc), "mkl_sparse_s_spmmd");    
		alpha_timing_end(&timer);    
		total_time += alpha_timing_elapsed_time(&timer);    
	}    
	printf("iter is %d, %s avg time : %lf[sec]\n",iter,"mkl_sparse_s_spmmd",total_time/iter);
    //alpha_timing_end(&timer);
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file, int thread_num, float **ret, size_t *ret_size, alphasparse_layout_t block_layout)
{
	...
	for(int i = 0;i<iter;i++){    
		alpha_clear_cache();    
		alpha_timing_start(&timer);    
		alpha_call_exit(alphasparse_s_spmmd_plain(transA, bsrA, bsrB, layout, C, ldc), "alphasparse_s_spmmd");    
		alpha_timing_end(&timer);    
		total_time += alpha_timing_elapsed_time(&timer);    
	}    
	printf("iter is %d, %s avg time : %lf[sec]\n",iter,"alphasparse_s_spmmd_plain",total_time/iter);;
    //alpha_timing_end(&timer);
...
int main(int argc, const char *argv[])
{
    ...
    { // block row major
        ...
        if (check)
        {
            ...
            status = check_s((float *)mkl_C, size_mkl_C, (float *)alpha_C, size_alpha_C);
            alpha_free(mkl_C);
        }
    }
