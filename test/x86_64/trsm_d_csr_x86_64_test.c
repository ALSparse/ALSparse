...
static void alpha_trsm(const int argc, const char *argv[], const char *file, int thread_num, const double alpha, double **ret_x, size_t *ret_size_x, ALPHA_INT *ret_ldx, double **ret, size_t *size, ALPHA_INT *ret_ldc)
{
    ALPHA_INT m, k, nnz;
    ...
    alpha_timer_t timer;
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_d_trsm_plain(transA, alpha, csrA, descr, layout, x, columns, ldx, y, ldy), "alphasparse_d_trsm_plain");
    alpha_timing_end(&timer);
    printf("%lf,%lf", alpha_timing_elapsed_time(&timer), alpha_timing_gflops(&timer, (double)nnz * k * 2 + m * k));
...
//        status = check_d(double *)mkl_y, size_mkl_y * 2, (double *)alpha_y, size_alpha_y * 2);
        status = check_d_l3((double *)mkl_y, ldc, size_mkl_y, alpha_y, ldc, size_alpha_y, NULL, alpha_x, ldx, alpha_y, ldc, alpha_alpha, alpha_alpha, argc, argv);
        alpha_free(mkl_y);
    }
    printf("\n");
    alpha_free(alpha_y);
