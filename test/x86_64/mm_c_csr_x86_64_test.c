...
static void alpha_mm(const int argc, const char *argv[], const char *file, int thread_num, ALPHA_Complex8 alpha, ALPHA_Complex8 beta, ALPHA_Complex8 **ret_x, size_t *ret_size_x, ALPHA_INT *ret_ldx, ALPHA_Complex8 **ret_y, size_t *ret_size_y, ALPHA_INT *ret_ldy)
{
    ALPHA_INT m, k, nnz;
...
//        check_s(float *)mkl_y, size_mkl_y * 2, (float *)alpha_y, size_alpha_y * 2);
        status = check_c_l3((ALPHA_Complex8 *)mkl_y, ldy, size_mkl_y, alpha_y, ldy, size_alpha_y, NULL, alpha_x, ldx, alpha_y, ldy, alpha, beta, argc, argv);
        alpha_free(mkl_y);
    }
    printf("\n");
    alpha_free(alpha_y);
