...
static void alpha_mm(const int argc, const char *argv[], const char *file, int thread_num, float alpha, float beta, float **ret_x, size_t *ret_size_x, ALPHA_INT *ret_ldx, float **ret_y, size_t *ret_size_y, ALPHA_INT *ret_ldy)
{
    ALPHA_INT m, k, nnz;
    ...
    alpha_call_exit(alphasparse_s_mm(transA, alpha, csr, descr, layout, x, columns, ldx, beta, y, ldy), "alphasparse_s_mm");
...
//        check_s(float *)mkl_y, size_mkl_y * 2, (float *)alpha_y, size_alpha_y * 2);
        printf("\n");
        status = check_s_l3((float *)mkl_y, ldy, size_mkl_y, alpha_y, ldy, size_alpha_y, NULL, alpha_x, ldx, alpha_y, ldy, alpha, beta, argc, argv);
        alpha_free(mkl_y);
    }
    printf("\n");
    alpha_free(alpha_y);
