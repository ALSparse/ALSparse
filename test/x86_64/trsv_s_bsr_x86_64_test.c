...
static void mkl_trsv(const int argc, const char *argv[], const char *file, int thread_num, const float alpha, float **ret, size_t *size)
{
    ...
    mkl_call_exit(mkl_sparse_convert_bsr(cooA, 4, SPARSE_LAYOUT_ROW_MAJOR, SPARSE_OPERATION_NON_TRANSPOSE, &csrA), "mkl_sparse_convert_bsr");
...
static void alpha_trsv(const int argc, const char *argv[], const char *file, int thread_num, const float alpha, float **ret, size_t *size, bool flag)
{
    ALPHA_INT m, k, nnz;
...
int main(int argc, const char *argv[])
{
    ...
    if (check)
    {
        ...
        status = check_s(mkl_y_col, size_mkl_y_col, alpha_y_col, size_alpha_y_col);
