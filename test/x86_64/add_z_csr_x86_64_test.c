...
static void mkl_add(const int argc, const char *argv[], const char *file, MKL_Complex16 alpha, sparse_index_base_t *ret_index, MKL_INT *ret_rows, MKL_INT *ret_cols, MKL_INT **ret_rows_start, MKL_INT **ret_rows_end, MKL_INT **ret_col_index, MKL_Complex16 **ret_values, int thread_num)
{
    MKL_INT m, k, nnz;
    ...
    mkl_call_exit(mkl_sparse_z_add(transA, csr, alpha, csrt, &result), "mkl_sparse_z_add");
    ...
    mkl_call_exit(mkl_sparse_z_export_csr(result, ret_index, ret_rows, ret_cols, ret_rows_start, ret_rows_end, ret_col_index, ret_values), "mkl_sparse_z_export_csr");
...
//        status = check_d(double *)mkl_values, 2 * mkl_nnz, (double *)alpha_values, 2 * alpha_nnz);
        status = check_z_l3((ALPHA_Complex16 *)mkl_values, 0, mkl_nnz, alpha_values, 0, alpha_nnz, alpha_col_index, NULL, 0, alpha_values, 0, alpha, beta, argc, argv);
    }
    printf("\n");
    return status;
}