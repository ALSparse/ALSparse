...
static void mkl_trsm(const int argc, const char *argv[], const char *file, int thread_num, const MKL_Complex8 alpha, MKL_Complex8 **ret, size_t *size)
{
    ...
    mkl_call_exit(mkl_sparse_c_trsm(transA, alpha, csrA, descr, layout, x, columns, ldx, y, ldy), "mkl_sparse_c_trsm");
...
static void alpha_trsm(const int argc, const char *argv[], const char *file, int thread_num, const ALPHA_Complex8 alpha, ALPHA_Complex8 **ret_x, size_t *ret_size_x, ALPHA_INT *ret_ldx, ALPHA_Complex8 **ret, size_t *size, ALPHA_INT *ret_ldc)
{
    ALPHA_INT m, k, nnz;
...
//        status = check_s(float *)mkl_y, size_mkl_y * 2, (float *)alpha_y, size_alpha_y * 2);
        status = check_c_l3((ALPHA_Complex8 *)mkl_y, ldc, size_mkl_y, alpha_y, ldc, size_alpha_y, NULL, alpha_x, ldx, alpha_y, ldc, alpha_alpha, alpha_alpha, argc, argv);
        alpha_free(mkl_y);
    }
    printf("\n");
    alpha_free(alpha_y);
