...
static void alpha_spmmd_plain(const int argc, const char *argv[], const char *file, int thread_num, float **ret, size_t *ret_size, alphasparse_layout_t block_layout)
{
    ...
    alphasparse_s_spmmd_plain(transA, bsrA, bsrB, layout, C, ldc);
...
static void alpha_spmmd(const int argc, const char *argv[], const char *file, int thread_num, float **ret, size_t *ret_size, alphasparse_layout_t block_layout)
{
    alpha_set_thread_num(thread_num);
    ALPHA_INT m, k, nnz;
    ALPHA_INT *row_index, *col_index;
    ...
    alpha_timer_t timer;
    alpha_timing_start(&timer);
    alphasparse_s_spmmd(transA, bsrA, bsrB, layout, C, ldc);
    alpha_timing_end(&timer);
    alpha_timing_elaped_time_print(&timer, "alphasparse_s_spmmd");
    alphasparse_destroy(cooA);
    alphasparse_destroy(bsrA);
...
int main(int argc, const char *argv[])
{
    ...
    { // block row major
        ...
        if (check)
        {
            ...
            status = check_s((float *)alpha_plain_C,size_alpha_plain_C, (float *)alpha_C,size_alpha_C);
