...
static void alpha_mv()
{
    ...
    for (int i = 0; i < iter; i++)
    {
        ...
        alpha_call_exit(alphasparse_c_mv(transA, alpha, csrA, descr, x, beta, icty), "alphasparse_c_mv");
        alpha_timing_end(&timer);
        total_time += alpha_timing_elapsed_time(&timer);
    }

    printf("%s time : %lf[sec]\n","alphasparse_c_mv",(total_time/iter));
...
static void alpha_mv_plain()
{
    ...
    for (int i = 0; i < iter; i++)
    {
        // alpha_clear_cache();
        alpha_timing_start(&timer);
        alpha_call_exit(alphasparse_c_mv_plain(transA, alpha, csrA, descr, x, beta, icty_plain), "alphasparse_c_mv_plain");
        alpha_timing_end(&timer);
        total_time += alpha_timing_elapsed_time(&timer);
    }
    printf("%s time : %lf[sec]\n","alphasparse_c_mv_plain",(total_time/iter));
...
int main(int argc, const char *argv[])
{
    ...
    // init x y
    x = alpha_malloc(sizex * sizeof(ALPHA_Complex8));
    icty = alpha_malloc(sizey * sizeof(ALPHA_Complex8));
    icty_plain = alpha_malloc(sizey * sizeof(ALPHA_Complex8));
    ...
    if (check)
    {
        ...
        alpha_mv_plain();
        status = check_c_l2((ALPHA_Complex8*)icty, sizey, icty_plain, sizey, x, NULL, alpha, beta, argc, argv);
