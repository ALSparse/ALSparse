...
void alpha_trsm(const int argc, const char *argv[], ALPHA_INT m, ALPHA_INT n, ALPHA_INT nnz, ALPHA_INT *row_index, ALPHA_INT *col_index, double *values, double alpha, double *x, ALPHA_INT columns, ALPHA_INT ldx, double *y, ALPHA_INT ldy, int thread_num)
{
    ...
    alpha_call_exit(alphasparse_d_trsm(transA, alpha, csrA, descr, layout, x, columns, ldx, y, ldy), "alphasparse_d_trsm");
...
int main(int argc,const char *argv[])
{
    ...
    if (check)
    {
        ...
        printf("\n");
        status = check_d_l3((double *)alpha_y, ldy, m * columns, plain_y, ldy, m * columns, NULL, x, ldx, alpha_y, ldy, alpha, alpha, argc, argv);
        // status = check_d(plain_y, m * columns, alpha_y, m * columns);
    }
    printf("\n");
    alpha_free(x);
    alpha_free(alpha_y);
