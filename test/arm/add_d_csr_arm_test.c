...
static void alpha_add_plain(const int argc, const char *argv[], const char *file, double alpha, alphasparse_index_base_t *ret_index, ALPHA_INT *ret_rows, ALPHA_INT *ret_cols, ALPHA_INT **ret_rows_start, ALPHA_INT **ret_rows_end, ALPHA_INT **ret_col_index, double **ret_values, int thread_num)
{
    ALPHA_INT m, k, nnz;
...
int main(int argc, const char *argv[])
{
    ...
    if (check)
    {
        ...
        printf(",");
        // status = check_d(plain_values, plain_nnz, alpha_values, alpha_nnz);
        status = check_d_l3((double *)alpha_values, 0, alpha_nnz, alpha_values, 0, plain_nnz, alpha_col_index, NULL, 0, alpha_values, 0, alpha, beta, argc, argv);
    }
    printf("\n");
