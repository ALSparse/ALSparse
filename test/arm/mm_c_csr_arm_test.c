...
void alpha_mm()
{
    ...
    // 稀疏矩阵乘稠密矩阵
    alpha_call_exit(alphasparse_c_mm(transA, alpha, csr, descr, layout, x, columns, ldx, beta, icty, ldy), "alphasparse_c_mm");
    alpha_timing_end(&timer);
    alpha_timing_elaped_time_print(&timer, "alphasparse_c_mm");
    alpha_timing_gflops_print(&timer, ops, "alphasparse_c_mm");
    // 释放稀疏矩阵
...
void alpha_mm_plain()
{
    ...
    alpha_timer_t timer;
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_c_mm_plain(transA, alpha, csr, descr, layout, x, columns, ldx, beta, icty_plain, ldy), "alphasparse_c_mm_plain");
    alpha_timing_end(&timer);

    alpha_timing_elaped_time_print(&timer, "alphasparse_c_mm_plain");
    alpha_timing_gflops_print(&timer, ops, "alphasparse_c_mm_plain");
    alphasparse_destroy(coo);
    alphasparse_destroy(csr);
}
...
int main(int argc, const char *argv[])
{
    ...
    x = (ALPHA_Complex8 *)alpha_memalign(sizeof(ALPHA_Complex8) * sizex, DEFAULT_ALIGNMENT);
    icty = (ALPHA_Complex8 *)alpha_memalign(sizeof(ALPHA_Complex8) * sizey, DEFAULT_ALIGNMENT);
    icty_plain = (ALPHA_Complex8 *)alpha_memalign(sizeof(ALPHA_Complex8) * sizey, DEFAULT_ALIGNMENT);

    alpha_fill_random_c(x, 2, sizex);
    ...
    if (check)
    {
        alpha_fill_random_c(icty_plain, 1, sizey);
        alpha_mm_plain();
        status = check_c_l3((ALPHA_Complex8 *)icty, ldy, sizey, icty_plain, ldy, sizey, NULL, x, ldx, icty, ldy, alpha, beta, argc, argv);
        // status = check_c(icty, sizey, icty_plain, sizey);
    }
    printf("\n");
    alpha_free(x);
    alpha_free(icty);
