...
static void alpha_spmmd(const int argc, const char *argv[], ALPHA_INT mA, ALPHA_INT nA, ALPHA_INT mB, ALPHA_INT nB, \
    ALPHA_INT nnzA, ALPHA_INT *row_indexA, ALPHA_INT *col_indexA, ALPHA_Complex8 *valuesA, \
    ALPHA_INT nnzB, ALPHA_INT *row_indexB, ALPHA_INT *col_indexB, ALPHA_Complex8 *valuesB, \
    ALPHA_Complex8 *C, ALPHA_INT ldc, ALPHA_INT thread_num)
{
    ...
    alpha_call_exit(alphasparse_c_create_coo(&cooA, ALPHA_SPARSE_INDEX_BASE_ZERO, mA, nA, nnzA, row_indexA, col_indexA, valuesA), "alphasparse_c_create_coo");
    ...
    alpha_call_exit(alphasparse_c_create_coo(&cooB, ALPHA_SPARSE_INDEX_BASE_ZERO, mB, nB, nnzB, row_indexB, col_indexB, valuesB), "alphasparse_c_create_coo");
    alpha_call_exit(alphasparse_convert_csc(cooB, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &cscB), "alphasparse_convert_csc");

    alpha_timer_t timer;
    alpha_timing_start(&timer);

    alpha_call_exit(alphasparse_c_spmmd(transA, cscA, cscB, layout, C, ldc), "alphasparse_c_spmmd");
    ...
    ALPHA_Complex8 *C, ALPHA_INT ldc, ALPHA_INT thread_num)
{
    ...
    alpha_call_exit(alphasparse_c_spmmd_plain(transA, cscA, cscA, layout, C, ldc), "alphasparse_c_spmmd_plain");
...
int main(int argc,const char *argv[])
{
    ...
    // read coo
    alpha_read_coo_c(fileA, &mA, &nA, &nnzA, &row_indexA, &col_indexA, &valuesA);
    alpha_read_coo_c(fileB, &mB, &nB, &nnzB, &row_indexB, &col_indexB, &valuesB);
    ...
    ALPHA_Complex8 *alpha_C = alpha_malloc(sizeof(ALPHA_Complex8) * (C_m * C_n));
    ALPHA_Complex8 *alpha_C_plain = alpha_malloc(sizeof(ALPHA_Complex8) * (C_m * C_n));

    alpha_fill_random_c(valuesA, 0, nnzA);
    alpha_fill_random_c(valuesB, 0, nnzB);
    ...
    if (check)
    {
        ...
        status = check_c(alpha_C, C_m * C_n, alpha_C_plain, C_m * C_n);
    }
