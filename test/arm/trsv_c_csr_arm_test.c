...
void plain_trsv(const int argc, const char *argv[], ALPHA_INT m, ALPHA_INT n, ALPHA_INT nnz, ALPHA_INT *row_index, ALPHA_INT *col_index, ALPHA_Complex8 *values, ALPHA_Complex8 *x, ALPHA_Complex8 alpha, ALPHA_Complex8 *y, int thread_num)
{
...
void alpha_trsv(const int argc, const char *argv[], ALPHA_INT m, ALPHA_INT n, ALPHA_INT nnz, ALPHA_INT *row_index, ALPHA_INT *col_index, ALPHA_Complex8 *values, ALPHA_Complex8 *x, ALPHA_Complex8 alpha, ALPHA_Complex8 *y, int thread_num)
{
    alpha_set_thread_num(thread_num);
    ...
    alpha_call_exit(alphasparse_c_trsv(transA, alpha, csrA, descr, x, y), "alphasparse_c_trsv");
...
int main(int argc,const char *argv[])
{
    ...
    // init x y
    x = alpha_malloc(k * sizeof(ALPHA_Complex8));
    alpha_y = alpha_malloc(m * sizeof(ALPHA_Complex8));
    plain_y = alpha_malloc(m * sizeof(ALPHA_Complex8));

    // alpha_fill_random_s(x, 0, k);
    alpha_fill_random_c(x, 1, k);
    ...
    if (check)
    {
        ...
        printf("\n");
        status = check_c_l2((ALPHA_Complex8 *)alpha_y, m, plain_y, m, x, NULL, alpha, alpha, argc, argv);
        // status = check_c(plain_y, m, alpha_y, m);
    }
    printf("\n");
    alpha_free(x);
    alpha_free(alpha_y);
