...
static void alpha_mv(const int argc, const char *argv[],  ALPHA_INT m,  ALPHA_INT n,  ALPHA_INT nnz,  ALPHA_INT *row_index,  ALPHA_INT *col_index,  double *values,  double *x,  double alpha,  double beta, double *y, ALPHA_INT thread_num)
{
    ...
    alpha_call_exit(alphasparse_d_mv(transA, 2, cscA, descr, x, 3, y), "alphasparse_d_mv");

    alpha_timing_end(&timer);
    alpha_timing_elaped_time_print(&timer, "alpha_mv_d_csc time");
        // save run time
    FILE* fp;
    fp = fopen("1.txt", "a+");
    if (fp) {
...
static void alpha_mv_plain(const int argc, const char *argv[],  ALPHA_INT m,  ALPHA_INT n,  ALPHA_INT nnz,  ALPHA_INT *row_index,  ALPHA_INT *col_index,  double *values,  double *x,  double alpha,  double beta, double *y, ALPHA_INT thread_num)
{
    ...
    alpha_call_exit(alphasparse_d_mv_plain(transA, 2, cscA, descr, x, 3, y), "alphasparse_d_mv_plain");
...
int main(int argc,const char *argv[])
{
    ...
    // init x y
    double *x = alpha_malloc(kk * sizeof(double));
    double *alpha_y = alpha_malloc(mm * sizeof(double));
    double *alpha_y_plain = alpha_malloc(mm * sizeof(double));

    alpha_fill_random_d(x, 0, kk);
    alpha_fill_random_d(alpha_y, 1, mm);
