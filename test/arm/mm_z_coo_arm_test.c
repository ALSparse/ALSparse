...
void alpha_mm(const int argc, const char *argv[], ALPHA_INT m, ALPHA_INT n, ALPHA_INT nnz, ALPHA_INT *row_index, ALPHA_INT *col_index, ALPHA_Complex16 *values, ALPHA_Complex16 alpha, ALPHA_Complex16 beta, ALPHA_Complex16 *x, ALPHA_INT columns, ALPHA_INT ldx, ALPHA_Complex16 *y, ALPHA_INT ldy, int thread_num)
{
    ...
    alpha_call_exit(alphasparse_z_mm(transA, alpha, coo, descr, layout, x, columns, ldx, beta, y, ldy), "alphasparse_z_mm");
...
void alpha_mm_plain(const int argc, const char *argv[], ALPHA_INT m, ALPHA_INT n, ALPHA_INT nnz, ALPHA_INT *row_index, ALPHA_INT *col_index, ALPHA_Complex16 *values, ALPHA_Complex16 alpha, ALPHA_Complex16 beta, ALPHA_Complex16 *x, ALPHA_INT columns, ALPHA_INT ldx, ALPHA_Complex16 *y, ALPHA_INT ldy, int thread_num)
{
    ...
    alpha_call_exit(alphasparse_z_mm_plain(transA, alpha, coo, descr, layout, x, columns, ldx, beta, y, ldy), "alphasparse_z_mm_plain");
...
int main(int argc,const char *argv[])
{
    ...
    ALPHA_Complex16 *x = alpha_memalign(sizeof(ALPHA_Complex16) * cols * columns, DEFAULT_ALIGNMENT);
    ALPHA_Complex16 *openspblasy = alpha_memalign(sizeof(ALPHA_Complex16) * rows * columns, DEFAULT_ALIGNMENT);
    ALPHA_Complex16 *openspblasy_plain = alpha_memalign(sizeof(ALPHA_Complex16) * rows * columns, DEFAULT_ALIGNMENT);

    alpha_fill_random_z(values, 2, nnz);
    alpha_fill_random_z(x, 2, cols * columns);
    alpha_fill_random_z(openspblasy, 1, rows * columns);
    ...
    if (check)
    {
        alpha_fill_random_z(openspblasy_plain, 1, rows * columns);
        alpha_mm_plain(argc, argv, m, k, nnz, row_index, col_index, values, alpha, beta, x, columns, ldx, openspblasy_plain, ldy, thread_num);
        status = check_z(openspblasy, rows * columns, openspblasy_plain, rows * columns);
    }
    alpha_free(x);
