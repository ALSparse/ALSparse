...
static void alpha_plain_spmm(const int argc, const char *argv[], ALPHA_INT m, ALPHA_INT n, ALPHA_INT nnz, ALPHA_INT *row_index, ALPHA_INT *col_index, ALPHA_Complex8 *values, alphasparse_index_base_t *ret_index, ALPHA_INT *ret_rows, ALPHA_INT *ret_cols, ALPHA_INT **ret_rows_start, ALPHA_INT **ret_rows_end, ALPHA_INT **ret_col_index, ALPHA_Complex8 **ret_values, int thread_num)
{
    alpha_set_thread_num(thread_num);
    alphasparse_matrix_t coo, csrA, csrB, result;
    const char *fileA = args_get_data_fileA(argc, argv);
    alpha_read_coo_c(fileA, &m, &n, &nnz, &row_index, &col_index, &values);
    ...
    alpha_read_coo_c(fileB, &m, &n, &nnz, &row_index, &col_index, &values);
...
int main(int argc, const char *argv[])
{
    ...
    ALPHA_Complex8 *alpha_plain_values;
    ...
    if (check)
    {
        ...
        status = check_c_l3((ALPHA_Complex8 * )alpha_values, 0, alpha_nnz, alpha_values, 0, alpha_plain_nnz, alpha_col_index, NULL, 0, alpha_plain_values, 0, alpha, beta, argc, argv);    
        // status = check_c(alpha_plain_values, alpha_plain_nnz, alpha_values, alpha_nnz);
    }
