...
void alpha_mm()
{
    ...
    // 稀疏矩阵乘稠密矩阵
    alpha_call_exit(alphasparse_z_mm(transA, alpha, csr, descr, layout, x, columns, ldx, beta, icty, ldy), "alphasparse_z_mm");
    alpha_timing_end(&timer);
    alpha_timing_elaped_time_print(&timer, "alphasparse_z_mm");
    alpha_timing_gflops_print(&timer, ops, "alphasparse_z_mm");
    // 释放稀疏矩阵
...
void alpha_mm_plain()
{
    ...
    alpha_timer_t timer;
    alpha_timing_start(&timer);
    alpha_call_exit(alphasparse_z_mm_plain(transA, alpha, csr, descr, layout, x, columns, ldx, beta, icty_plain, ldy), "alphasparse_z_mm_plain");
    alpha_timing_end(&timer);

    alpha_timing_elaped_time_print(&timer, "alphasparse_z_mm_plain");
    alpha_timing_gflops_print(&timer, ops, "alphasparse_z_mm_plain");
    alphasparse_destroy(coo);
    alphasparse_destroy(csr);
}
...
int main(int argc, const char *argv[])
{
    ...
    x = (ALPHA_Complex16 *)alpha_memalign(sizeof(ALPHA_Complex16) * sizex, DEFAULT_ALIGNMENT);
    icty = (ALPHA_Complex16 *)alpha_memalign(sizeof(ALPHA_Complex16) * sizey, DEFAULT_ALIGNMENT);
    icty_plain = (ALPHA_Complex16 *)alpha_memalign(sizeof(ALPHA_Complex16) * sizey, DEFAULT_ALIGNMENT);
    ...
    if (check)
    {
        ...
        alpha_mm_plain();
        status = check_z_l3((ALPHA_Complex16 *)icty, ldy, sizey, icty_plain, ldy, sizey, NULL, x, ldx, icty, ldy, alpha, beta, argc, argv);
