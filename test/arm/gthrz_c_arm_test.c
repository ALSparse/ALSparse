...
static void alpha_gthrz(const ALPHA_INT n, ALPHA_Complex8 * x, const ALPHA_INT *indx, ALPHA_Complex8 *y, int thread_num)
{
    ...
    alpha_call_exit(alphasparse_c_gthrz(n, y, x, indx), "alpha_c_gthrz");

    alpha_timing_end(&timer);
    alpha_timing_elaped_time_print(&timer, "alpha_c_gthrz");
}

static void alpha_gthrz_plain(const ALPHA_INT n, ALPHA_Complex8 * x, const ALPHA_INT *indx, ALPHA_Complex8 *y, int thread_num)
{
    alpha_set_thread_num(thread_num);
    alpha_timer_t timer;
    alpha_timing_start(&timer);

    alpha_call_exit(alphasparse_c_gthrz_plain(n, y, x, indx), "alpha_c_gthrz_plain");

    alpha_timing_end(&timer);
    alpha_timing_elaped_time_print(&timer, "alpha_c_gthrz_plain");
}
...
int main(int argc,const char *argv[])
{
    ...
    ALPHA_Complex8 *alpha_x_plain = alpha_memalign(sizeof(ALPHA_Complex8) * alpha_n, DEFAULT_ALIGNMENT);
    ...
    ALPHA_Complex8 *alpha_y_plain = alpha_memalign(sizeof(ALPHA_Complex8) * alpha_n * 20, DEFAULT_ALIGNMENT);

    alpha_fill_random_c(alpha_y, 1, alpha_n * 20);
    alpha_fill_random_c(alpha_y_plain, 1, alpha_n * 20);
    ...
    if (check)
    {
        ...
        status = check_c(alpha_x_plain, alpha_n, alpha_x, alpha_n);
    }
