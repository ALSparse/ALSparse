...
//   hipMemcpy(dmatC, matC_roc, sizeof(double) * C_cols * ldc,
//             hipMemcpyHostToDevice);
...
//     roc_call_exit(
//         rocsparse_dgebsrmm(handle, roclayout, roctransA, roctransB, m, n, k,
...
//   hipMemcpy(matC_roc, dmatC, sizeof(double) * C_cols * ldc,
...
static void alpha_mm_dcu()
{
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alpha_call_exit(
            alphasparse_dcu_d_gebsrmm(handle, layout, transA, transB, m, n, k, nnb, &halpha, descrA, dAval, dAptr, dAcol, bs_r, bs_c, dmatB, ldb, &hbeta, dmatC, ldc),
            "alphasparse_dcu_d_gebsrmm");
    }
...
int main(int argc, const char *argv[])
{
    ...
    // 获取bsr格式里的数据
    alpha_call_exit(
        alphasparse_d_export_gebsr(bsr, &bsr_index, &layout, &A_rowsb, &A_colsb, &bs_r, &bs_c, &bsr_row_ptr, &bsr_row_ptr_end, &bsr_col_index, &bsr_values),
