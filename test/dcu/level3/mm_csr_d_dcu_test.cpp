...
static void roc_mm_dcu()
{
    ...
    hipMemcpy(dmatB, matB, sizeof(double) * B_cols * ldb, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        rocsparse_dcsrmm(handle, roctransA, roctransB, m, n, k, nnz, &halpha, descrA, dAval, dAptr, dAcol, dmatB, ldb, &hbeta, dmatC, ldc);
    }
    ...
    for (int i = 0; i < trials; ++i) {
        for (int i = 0; i < batch_size; ++i) {
            // Call rocsparse csrmm
            rocsparse_status x = rocsparse_dcsrmm(
...
static void alpha_mm_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dmatB, matB, sizeof(double) * B_cols * ldb, hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alphasparse_dcu_d_csrmm(handle, transA, transB, layout, m, n, k, nnz, &halpha, descrA, dAval, dAptr, dAcol, dmatB, ldb, &hbeta, dmatC, ldc);
    }
