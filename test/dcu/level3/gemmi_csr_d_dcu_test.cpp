...
static void roc_mmi_dcu()
{
    ...
    hipMalloc((void **)&dmatA, sizeof(double) * C_k * lda);
    ...
    hipMemcpy(dmatA, matA, sizeof(double) * C_k * lda, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        roc_call_exit(rocsparse_dgemmi(handle, roctransA, roctransB, m, n, k, nnz, &halpha, dmatA, lda, descrB, dAval, dAptr, dAcol, &hbeta, dmatC, ldc),
                      "rocsparse_dgemmi");
    }
...
static void alpha_mmi_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(hipMalloc((void **)&dmatA, sizeof(double) * C_k * lda));
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dmatA, matA, sizeof(double) * C_k * lda, hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alpha_call_exit(alphasparse_dcu_d_gemmi(handle, transA, transB, m, n, k, nnz, &halpha, dmatA, lda, descrB, dAval, dAptr, dAcol, &hbeta, dmatC, ldc),
                        "alphasparse_dcu_d_gemmi");
    }
...
int main(int argc, const char *argv[])
{
    ...
    double *coo_values;
    // read coo
    alpha_read_coo_d(file, &B_rows, &B_cols, &rnnz, &coo_row_index, &coo_col_index, &coo_values);
    A_rows = args_get_columns(argc, argv, B_rows); // 稠密矩阵A的行数

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_d_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, B_rows, B_cols, rnnz, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取csr格式里的数据
    alpha_call_exit(
        alphasparse_d_export_csr(csr, &csr_index, &B_rows, &B_cols, &csr_row_ptr, &csr_row_ptr_end, &csr_col_index, &csr_values),
