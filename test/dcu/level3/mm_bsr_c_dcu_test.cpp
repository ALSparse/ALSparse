...
static void roc_mm_dcu()
{
    ...
    hipMemcpy(dmatB, matB, sizeof(rocsparse_float_complex) * C_k * ldb, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        rocsparse_cbsrmm(handle, roclayout, roctransA, roctransB, m, n, k, nnb, &halpha, descrA, dAval, dAptr, dAcol, bs, dmatB, ldb, &hbeta, dmatC, ldc);
    }
    ...
    for (int i = 0; i < trials; ++i) {
        for (int i = 0; i < batch_size; ++i) {
            // Call rocsparse bsrmm
            rocsparse_status x = rocsparse_cbsrmm(
...
static void alpha_mm_dcu()
{
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alphasparse_dcu_c_bsrmm(handle, layout, transA, transB, m, n, k, nnb, &halpha, descrA, dAval, dAptr, dAcol, bs, dmatB, ldb, &hbeta, dmatC, ldc);
    }
...
int main(int argc, const char *argv[])
{
    ...
    // 获取bsr格式里的数据
    alpha_call_exit(
        alphasparse_c_export_bsr(bsr, &bsr_index, &layout, &A_rowsb, &A_colsb, &bs, &bsr_row_ptr, &bsr_row_ptr_end, &bsr_col_index, &bsr_values),
