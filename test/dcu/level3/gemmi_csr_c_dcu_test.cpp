...
// parms for kernel
ALPHA_Complex8 *matA, *matC_ict, *matC_roc;
...
static void roc_mmi_dcu()
{
    ...
    rocsparse_float_complex *dmatA = NULL;
    ...
    hipMalloc((void **)&dmatA, sizeof(rocsparse_float_complex) * C_k * lda);
    ...
    hipMemcpy(dmatA, matA, sizeof(rocsparse_float_complex) * C_k * lda, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        roc_call_exit(rocsparse_cgemmi(handle, roctransA, roctransB, m, n, k, nnz, &halpha, dmatA, lda, descrB, dAval, dAptr, dAcol, &hbeta, dmatC, ldc),
                      "rocsparse_cgemmi");
    }
...
static void alpha_mmi_dcu()
{
    ...
    ALPHA_Complex8 *hAval =
        (ALPHA_Complex8 *)alpha_malloc(sizeof(ALPHA_Complex8) * nnz);
    ...
    PRINT_IF_HIP_ERROR(
        hipMalloc((void **)&dmatA, sizeof(ALPHA_Complex8) * C_k * lda));
    PRINT_IF_HIP_ERROR(
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dmatA, matA, sizeof(ALPHA_Complex8) * C_k * lda, hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alpha_call_exit(alphasparse_dcu_c_gemmi(handle, transA, transB, m, n, k, nnz, &halpha, dmatA, lda, descrB, dAval, dAptr, dAcol, &hbeta, dmatC, ldc),
                        "alphasparse_dcu_c_gemmi");
    }
...
int main(int argc, const char *argv[])
{
    ...
    // read coo
    alpha_read_coo_c(file, &B_rows, &B_cols, &rnnz, &coo_row_index, &coo_col_index, &coo_values);
    A_rows = args_get_columns(argc, argv, B_rows); // 稠密矩阵A的行数

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_c_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, B_rows, B_cols, rnnz, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取csr格式里的数据
    alpha_call_exit(
        alphasparse_c_export_csr(csr, &csr_index, &B_rows, &B_cols, &csr_row_ptr, &csr_row_ptr_end, &csr_col_index, &csr_values),
    ...
    // init B C
    matA     = (ALPHA_Complex8 *)alpha_malloc(C_k * lda * sizeof(ALPHA_Complex8));
    ...
    alpha_fill_random_c(matA, 0, C_k * lda);
