...
// parms for kernel
ALPHA_Complex8 *matB_roc, *matB_ict;
...
static void roc_trsm_dcu()
{
    ...
    hipMemcpy(dmatB, matB_roc, sizeof(rocsparse_float_complex) * B_cols * ldb, hipMemcpyHostToDevice);
    ...
    rocsparse_ccsrsm_analysis(handle, roctransA, roctransB, m, nrhs, nnz, &halpha, descrA, dAval, dAptr, dAcol, dmatB, ldb_, info, rocsparse_analysis_policy_reuse, rocsparse_solve_policy_auto, temp_buffer);
    time1 = (get_time_us() - time1) / (trials * batch_size * 1e3);

    // Warm up
    for (int i = 0; i < warm_up; ++i) {
        // Call rocsparse csrsm
        roc_call_exit(rocsparse_ccsrsm_solve(
    ...
    hipMemcpy(matB_roc, dmatB, sizeof(ALPHA_Complex8) * B_cols * ldb, hipMemcpyDeviceToHost);
...
static void alpha_trsm_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dmatB, matB_ict, sizeof(ALPHA_Complex8) * B_cols * ldb, hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alpha_call_exit(alphasparse_dcu_c_csrsm_solve(
    ...
    hipMemcpy(matB_ict, dmatB, sizeof(ALPHA_Complex8) * B_cols * ldb, hipMemcpyDeviceToHost);
...
int main(int argc, const char *argv[])
{
    ...
    // init B C
    matB_ict = (ALPHA_Complex8 *)alpha_malloc(B_cols * ldb * sizeof(ALPHA_Complex8));
    matB_roc = (ALPHA_Complex8 *)alpha_malloc(B_cols * ldb * sizeof(ALPHA_Complex8));

    alpha_fill_random_c(matB_ict, 1, B_cols * ldb);
    alpha_fill_random_c(matB_roc, 1, B_cols * ldb);
    ...
    if (check) {
        ...
        // }
        check_c((ALPHA_Complex8 *)matB_roc, B_cols * ldb, (ALPHA_Complex8 *)matB_ict, B_cols * ldb);
    }
    printf("\n");
