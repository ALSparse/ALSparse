...
// parms for kernel
ALPHA_Complex16 *matB, *matC_ict, *matC_roc;
...
//   hipMalloc((void **)&dmatB, sizeof(rocsparse_double_complex) * C_k * ldb);
...
//             hipMemcpyHostToDevice);
//   hipMemcpy(dAval, hAval.data(), sizeof(rocsparse_double_complex) * nnz,
//             hipMemcpyHostToDevice);
//   hipMemcpy(dmatB, matB, sizeof(rocsparse_double_complex) * C_k * ldb,
//             hipMemcpyHostToDevice);
//   hipMemcpy(dmatC, matC_roc, sizeof(rocsparse_double_complex) * C_cols * ldc,
//             hipMemcpyHostToDevice);
...
//     roc_call_exit(
//         rocsparse_zgebsrmm(handle, roclayout, roctransA, roctransB, m, n, k,
...
//   hipMemcpy(matC_roc, dmatC, sizeof(ALPHA_Complex16) * C_cols * ldc,
...
static void alpha_mm_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(
        hipMalloc((void **)&dmatB, sizeof(ALPHA_Complex16) * C_k * ldb));
    PRINT_IF_HIP_ERROR(
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dmatB, matB, sizeof(ALPHA_Complex16) * C_k * ldb, hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alpha_call_exit(
            alphasparse_dcu_z_gebsrmm(handle, layout, transA, transB, m, n, k, nnb, &halpha, descrA, dAval, dAptr, dAcol, bs_r, bs_c, dmatB, ldb, &hbeta, dmatC, ldc),
            "alphasparse_dcu_z_gebsrmm");
    }
...
int main(int argc, const char *argv[])
{
    ...
    // read coo
    alpha_read_coo_z(file, &A_rowsb, &A_colsb, &nnzb, &coo_row_index, &coo_col_index, &coo_values);
    columns = args_get_columns(argc, argv, A_colsb); // 默认C是方阵

    // 创建coo格式稀疏矩阵
    alpha_call_exit(alphasparse_z_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, A_rowsb, A_colsb, nnzb, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取bsr格式里的数据
    alpha_call_exit(
        alphasparse_z_export_gebsr(bsr, &bsr_index, &layout, &A_rowsb, &A_colsb, &bs_r, &bs_c, &bsr_row_ptr, &bsr_row_ptr_end, &bsr_col_index, &bsr_values),
    ...
    // init B C
    matB     = (ALPHA_Complex16 *)alpha_malloc(C_k * ldb * sizeof(ALPHA_Complex16));
    ...
    alpha_fill_random_z(matB, 0, C_k * ldb);
    ...
    //   //         endl;
    //   // }
    //   check_z((ALPHA_Complex16 *)matC_roc, C_cols * ldc, (ALPHA_Complex16 *)matC_ict,
    //           C_cols * ldc);
    // }
    printf("\n");
