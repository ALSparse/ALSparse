...
static void roc_mm_dcu()
{
    ...
    hipMemcpy(dmatB, matB, sizeof(rocsparse_double_complex) * B_cols * ldb, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        rocsparse_zcsrmm(handle, roctransA, roctransB, m, n, k, nnz, &halpha, descrA, dAval, dAptr, dAcol, dmatB, ldb, &hbeta, dmatC, ldc);
    }
    ...
    for (int i = 0; i < trials; ++i) {
        for (int i = 0; i < batch_size; ++i) {
            // Call rocsparse csrmm
            rocsparse_status x = rocsparse_zcsrmm(
...
static void alpha_mm_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dmatB, matB, sizeof(ALPHA_Complex16) * B_cols * ldb, hipMemcpyHostToDevice));
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alphasparse_dcu_z_csrmm(handle, transA, transB, layout, m, n, k, nnz, &halpha, descrA, dAval, dAptr, dAcol, dmatB, ldb, &hbeta, dmatC, ldc);
    }
...
int main(int argc, const char *argv[])
{
    ...
    // init B C
    matB     = (ALPHA_Complex16 *)alpha_malloc(B_cols * ldb * sizeof(ALPHA_Complex16));
    ...
    alpha_fill_random_z(matB, 0, B_cols * ldb);
