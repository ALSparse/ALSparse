...
alphasparse_matrix_t coo, csr, gebsr;
...
static void alpha_mv_dcu() {
  ...
  for (int i = 0; i < warm_up; ++i) {
    // Call alphasparse_dcu csrmv
    alpha_call_exit(alphasparse_s_mv_dcu(handle, transA, &alpha, gebsr, descrA, dx, &beta, dy), 
...
int main(int argc, const char *argv[]) {
  ...
  // coo2gebsr
  alpha_call_exit(alphasparse_convert_gebsr(coo, 2, 4, ALPHA_SPARSE_LAYOUT_ROW_MAJOR, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &gebsr),
      "alphasparse_convert_gebsr");
  
  // convert csr host ptr to device ptr
  alpha_call_exit(host2device_s_gebsr(gebsr), "host2device_s_gebsr");
