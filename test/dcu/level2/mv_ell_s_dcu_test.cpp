...
static void roc_mv_dcu()
{
    ...
    hipMalloc((void **)&dAval, sizeof(float) * m * ell_wdth);
    ...
    hipMemcpy(dAval, ell_values, sizeof(float) * m * ell_wdth, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        roc_call_exit(
            rocsparse_sellmv(handle, rocsparse_operation_none, m, n, &halpha, descrA, dAval, dAcol, ell_wdth, dx, &hbeta, dy),
            "rocsparse_sellmv");
    }
...
static void alpha_mv_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(hipMalloc((void **)&dAval, sizeof(float) * m * ell_width));
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dAval, ell_values, sizeof(float) * m * ell_width, hipMemcpyHostToDevice));
    PRINT_IF_HIP_ERROR(
        hipMemcpy(dx, x, sizeof(float) * sizex, hipMemcpyHostToDevice));
    PRINT_IF_HIP_ERROR(
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alphasparse_dcu_s_ellmv(handle, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, m, n, &halpha, descrA, dAval, dAcol, ell_width, dx, &hbeta, dy);
    }
...
int main(int argc, const char *argv[])
{
    ...
    float *coo_values;
    // read coo
    alpha_read_coo(file, &rm, &rk, &nnz, &coo_row_index, &coo_col_idx, &coo_values);

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_s_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, rm, rk, nnz, coo_row_index, coo_col_idx, coo_values),
        "alphasparse_s_create_coo");
    // 将稀疏矩阵从coo格式转换成ell格式
    alpha_call_exit(
        alphasparse_convert_ell(coo, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &ell),
        "alphasparse_convert_ell");
    alpha_call_exit(alphasparse_s_export_ell(ell, &ell_index, &rm, &rk, &ell_width, &ell_col_idx, &ell_values),
    ...
    rocy_coo = (float *)alpha_malloc(sizey * sizeof(float));

    alpha_fill_random_s(x, 4, sizex);
    alpha_fill_random_s(icty, 1, sizey);
    alpha_fill_random_s(rocy, 1, sizey);
    alpha_fill_random_s(rocy_coo, 1, sizey);
    ...
    if (check) {
        ...
        // check_s((float *)rocy_coo, sizey, (float *)icty, sizey);
    }
    printf("\n");
