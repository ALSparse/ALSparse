...
//     roc_call_exit(
//         rocsparse_sgebsrmv(handle, layout_roc, rocsparse_operation_none, m, n,
...
int main(int argc, const char *argv[])
{
    ...
    float *coo_values;
    // read coo
    alpha_read_coo(file, &rm, &rk, &nnz, &coo_row_index, &coo_col_index, &coo_values);

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_s_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, rm, rk, nnz, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取bsr格式里的数据
    alpha_call_exit(alphasparse_s_export_gebsr(gebsr, &gebsr_index, &layout, &rm, &rk, &blk_row, &blk_col, &gebsr_row_ptr, &gebsr_row_ptr_end, &gebsr_col_index, &gebsr_values),
