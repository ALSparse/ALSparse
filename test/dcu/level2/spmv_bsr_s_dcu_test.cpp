...
alphasparse_matrix_t coo, csr, bsr;
...
static void alpha_mv_dcu() {
  ...
  for (int i = 0; i < warm_up; ++i) {
    // Call alphasparse_dcu csrmv
    alpha_call_exit(alphasparse_s_mv_dcu(handle, transA, &alpha, bsr, descrA, dx, &beta, dy), 
...
int main(int argc, const char *argv[]) {
  ...
  // coo2bsr
  alpha_call_exit(alphasparse_convert_bsr(coo, bs, ALPHA_SPARSE_LAYOUT_ROW_MAJOR, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &bsr),
      "alphasparse_convert_bsr");
  
  // convert csr host ptr to device ptr
  alpha_call_exit(host2device_s_bsr(bsr), "host2device_s_bsr");
