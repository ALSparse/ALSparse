...
static void roc_mv_dcu()
{
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        rocsparse_zbsrmv(handle, layout_roc, rocsparse_operation_none, m, n, nnzb, &halpha, descrA, dAval, dAptr, dAcol, bs, dx, &hbeta, dy);
    }
...
static void alpha_mv_dcu()
{
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alphasparse_dcu_z_bsrmv(handle, layout, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, m, n, nnzb, &halpha, descrA, dAval, dAptr, dAcol, bs, dx, &hbeta, dy);
    }
...
int main(int argc, const char *argv[])
{
    ...
    // read coo
    alpha_read_coo_z(file, &rm, &rk, &nnzb, &coo_row_index, &coo_col_index, &coo_values);

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_z_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, rm, rk, nnzb, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取bsr格式里的数据
    alpha_call_exit(alphasparse_z_export_bsr(bsr, &bsr_index, &layout, &rm, &rk, &bs, &bsr_row_ptr, &bsr_row_ptr_end, &bsr_col_index, &bsr_values),
