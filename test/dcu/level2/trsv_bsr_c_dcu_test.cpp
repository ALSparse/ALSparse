...
static void roc_trsv_dcu()
{
    ...
    std::vector<ALPHA_Complex8> hAval(nnz * bsr_dim * bsr_dim);
    ...
    hipMalloc((void **)&dAval,
              sizeof(rocsparse_float_complex) * nnz * bsr_dim * bsr_dim);
    ...
    hipMemcpy(dAval, hAval.data(), sizeof(rocsparse_float_complex) * nnz * bsr_dim * bsr_dim, hipMemcpyHostToDevice);
    ...
    rocsparse_float_complex halpha;
    halpha.x = alpha.real;
    halpha.y = alpha.imag;
    ...
    roc_call_exit(rocsparse_cbsrsv_buffer_size(handle, roclayout, roctransA, mb, nnzb, descrA, dAval, dAptr, dAcol, bsr_dim, info, &buffer_size),
    ...
    roc_call_exit(
        rocsparse_cbsrsv_analysis(handle, roclayout, roctransA, mb, nnzb, descrA, dAval, dAptr, dAcol, bsr_dim, info, rocsparse_analysis_policy_reuse, rocsparse_solve_policy_auto, temp_buffer),
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        roc_call_exit(
            rocsparse_cbsrsv_solve(handle, roclayout, roctransA, mb, nnzb, &halpha, descrA, dAval, dAptr, dAcol, bsr_dim, info, dx, dy, rocsparse_solve_policy_auto, temp_buffer),
    ...
    for (int i = 0; i < trials; ++i) {
        for (int i = 0; i < batch_size; ++i) {
            // Call rocsparse bsrtrsv
            roc_call_exit(rocsparse_cbsrsv_solve(
...
static void alpha_trsv_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(hipMalloc((void **)&dAval,
                                 sizeof(ALPHA_Complex8) * nnz * bsr_dim * bsr_dim));
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dAval, bsr_values, sizeof(ALPHA_Complex8) * nnz * bsr_dim * bsr_dim, hipMemcpyHostToDevice));
    PRINT_IF_HIP_ERROR(
...
int main(int argc, const char *argv[])
{
    ...
    // read coo
    alpha_read_coo_c(file, &mb, &nb, &nnzb, &coo_row_index, &coo_col_index, &coo_values);

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_c_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, mb, nb, nnzb, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取bsr格式里的数据
    alpha_call_exit(alphasparse_c_export_bsr(
