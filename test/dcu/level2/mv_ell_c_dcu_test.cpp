...
ALPHA_Complex8 *ell_values;

// parms for kernel
ALPHA_Complex8 *x;
ALPHA_Complex8 *icty;
ALPHA_Complex8 *rocy, *rocy_coo;
...
static void roc_mv_dcu()
{
    ...
    hipMalloc((void **)&dAval, sizeof(rocsparse_float_complex) * m * ell_wdth);
    ...
    hipMemcpy(dAval, ell_values, sizeof(rocsparse_float_complex) * m * ell_wdth, hipMemcpyHostToDevice);
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        roc_call_exit(
            rocsparse_cellmv(handle, rocsparse_operation_none, m, n, &halpha, descrA, dAval, dAcol, ell_wdth, dx, &hbeta, dy),
            "rocsparse_cellmv");
    }
...
static void alpha_mv_dcu()
{
    ...
    PRINT_IF_HIP_ERROR(
        hipMalloc((void **)&dAval, sizeof(ALPHA_Complex8) * m * ell_width));
    ...
    PRINT_IF_HIP_ERROR(hipMemcpy(dAval, ell_values, sizeof(ALPHA_Complex8) * m * ell_width, hipMemcpyHostToDevice));
    PRINT_IF_HIP_ERROR(
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alphasparse_dcu_c_ellmv(handle, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, m, n, &halpha, descrA, dAval, dAcol, ell_width, dx, &hbeta, dy);
    }
...
int main(int argc, const char *argv[])
{
    ...
    // read coo
    alpha_read_coo_c(file, &rm, &rk, &nnz, &coo_row_index, &coo_col_idx, &coo_values);

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_c_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, rm, rk, nnz, coo_row_index, coo_col_idx, coo_values),
        "alphasparse_c_create_coo");
    // 将稀疏矩阵从coo格式转换成ell格式
    alpha_call_exit(
        alphasparse_convert_ell(coo, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &ell),
        "alphasparse_convert_ell");
    alpha_call_exit(alphasparse_c_export_ell(ell, &ell_index, &rm, &rk, &ell_width, &ell_col_idx, &ell_values),
    ...
    rocy_coo = (ALPHA_Complex8 *)alpha_malloc(sizey * sizeof(ALPHA_Complex8));

    alpha_fill_random_c(x, 4, sizex);
    alpha_fill_random_c(icty, 1, sizey);
    alpha_fill_random_c(rocy, 1, sizey);
    alpha_fill_random_c(rocy_coo, 1, sizey);
    ...
    if (check) {
        ...
        // check_c((ALPHA_Complex8 *)rocy_coo, sizey, (ALPHA_Complex8 *)icty, sizey);
    }
    printf("\n");
