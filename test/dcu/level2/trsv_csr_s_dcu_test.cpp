...
static void roc_trsv_dcu()
{
    ...
    rocsparse_scsrsv_buffer_size(handle, roctransA, m, nnz, descrA, dAval, dAptr, dAcol, info, &buffer_size);
    ...
    roc_call_exit(
        rocsparse_scsrsv_analysis(handle, roctransA, m, nnz, descrA, dAval, dAptr, dAcol, info, rocsparse_analysis_policy_reuse, rocsparse_solve_policy_auto, temp_buffer),
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        roc_call_exit(
            rocsparse_scsrsv_solve(handle, roctransA, m, nnz, &halpha, descrA, dAval, dAptr, dAcol, info, dx, dy, rocsparse_solve_policy_auto, temp_buffer),
...
static void alpha_trsv_dcu()
{
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alpha_call_exit(
            alphasparse_dcu_s_csrsv_solve(handle, transA, m, nnz, &halpha, descrA, dAval, dAptr, dAcol, NULL, dx, dy, ALPHA_SPARSE_DCU_SOLVE_POLICY_AUTO, NULL),
            "alphasparse_dcu_s_csrsv_solve");
    }
...
int main(int argc, const char *argv[])
{
    ...
    float *coo_values;
    // read coo
    alpha_read_coo(file, &rm, &rk, &rnnz, &coo_row_index, &coo_col_index, &coo_values);
    ...
    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_s_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, rm, rk, rnnz, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取csr格式里的数据
    alpha_call_exit(
        alphasparse_s_export_csr(csr, &csr_index, &rm, &rk, &csr_row_ptr, &csr_row_ptr_end, &csr_col_index, &csr_values),
