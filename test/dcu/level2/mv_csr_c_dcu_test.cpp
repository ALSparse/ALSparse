...
static void roc_mv_dcu()
{
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        roc_call_exit(
            rocsparse_ccsrmv(handle, rocsparse_operation_none, m, n, nnz, &halpha, descrA, dAval, dAptr, dAcol, nullptr, dx, &hbeta, dy),
            "rocsparse_scsrmv");
    }
    ...
    for (int i = 0; i < trials; ++i) {
        // Call rocsparse csrmv
        roc_call_exit(rocsparse_ccsrmv(handle, rocsparse_operation_none, m, n, nnz, &halpha, descrA, dAval, dAptr, dAcol, nullptr, dx, &hbeta, dy),
                      "rocsparse_scsrmv");
    ...
    double bandwidth = static_cast<double>(sizeof(rocsparse_float_complex) * (2 * m + nnz) + sizeof(rocsparse_int) * (m + 1 + nnz)) / time / 1e6;
    ...
    hipMemcpy(rocy, dy, sizeof(rocsparse_float_complex) * sizey, hipMemcpyDeviceToHost);
...
static void alpha_mv_dcu()
{
    ...
    for (int i = 0; i < warm_up; ++i) {
        ...
        alpha_call_exit(
            alphasparse_dcu_c_csrmv(handle, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, m, n, nnz, &halpha, descrA, dAval, dAptr, dAcol, info, dx, &hbeta, dy),
            "alphasparse_dcu_c_csrmv");
    }
    ...
    time             = (get_time_us() - time) / (trials * 1e3);
    double bandwidth = static_cast<double>(sizeof(ALPHA_Complex8)) * (2 * m + nnz) + sizeof(ALPHA_INT) * (m + 1 + nnz) / time / 1e6;
    double gflops    = static_cast<double>(2 * nnz) / time / 1e6;
...
int main(int argc, const char *argv[])
{
    ...
    // read coo
    alpha_read_coo_c(file, &rm, &rk, &rnnz, &coo_row_index, &coo_col_index, &coo_values);

    // 创建coo格式稀疏矩阵
    alpha_call_exit(
        alphasparse_c_create_coo(&coo, ALPHA_SPARSE_INDEX_BASE_ZERO, rm, rk, rnnz, coo_row_index, coo_col_index, coo_values),
    ...
    // 获取csr格式里的数据
    alpha_call_exit(
        alphasparse_c_export_csr(csr, &csr_index, &rm, &rk, &csr_row_ptr, &csr_row_ptr_end, &csr_col_index, &csr_values),
