...
alphasparse_matrix_t coo, csr, hyb;
...
static void alpha_mv_dcu() {
  ...
  for (int i = 0; i < warm_up; ++i) {
    // Call alphasparse_dcu csrmv
    alpha_call_exit(alphasparse_s_mv_dcu(handle, transA, &alpha, hyb, descrA, dx, &beta, dy), 
...
int main(int argc, const char *argv[]) {
  ...
  // coo2hyb
  alpha_call_exit(alphasparse_convert_hyb(coo, ALPHA_SPARSE_OPERATION_NON_TRANSPOSE, &hyb),
      "alphasparse_convert_hyb");
  // convert csr host ptr to device ptr
  alpha_call_exit(host2device_s_hyb(hyb), "host2device_s_hyb");
