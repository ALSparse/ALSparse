...
static alphasparse_status_t (*dcu_gemv_bsr_operation[])(alphasparse_dcu_handle_t handle,
                                                        ...
                                                        ALPHA_Number *y) = {
    dcu_gemv_bsr,
    NULL, //dcu_gemv_bsr_trans,
#ifdef COMPLEX
    NULL, //dcu_gemv_bsr_conj,
#endif
};
      ...
      ALPHA_Number *y)
{
    ...
    if (descr->type == ALPHA_SPARSE_MATRIX_TYPE_GENERAL) {
        check_null_return(dcu_gemv_bsr_operation[operation], ALPHA_SPARSE_STATUS_NOT_SUPPORTED);
        return dcu_gemv_bsr_operation[operation](handle, dir, mb, nb, nnzb, l_alpha, bsr_val, bsr_row_ptr, bsr_col_ind, bsr_dim, x, l_beta, y);
    } else {
        // doto
