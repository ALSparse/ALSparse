                                           ...
                                           alphasparse_matrix_t *dest)
{
    ...
    *dest = dest_;
    ...
    {
        status = convert_csr_datatype_format((const alpha_internal_spmat *)source->mat, (alpha_internal_spmat **)&dest_->mat, source->datatype, source->format);

        if(source->exe != ALPHA_SPARSE_EXE_HOST)
            host2device_csr((alphasparse_matrix_t)dest_);
