/** 
 * @Author: Zjj
 * @Date: 2020-05-11 11:13:25
 * @LastEditors: Zjj
 * @LastEditTime: 2020-05-11 20:57:31
 */
...
{
#ifdef DEBUG
    printf("kernel trsm_bsr_n_hi_col_plain called \n");
#endif
        ...
        for (ALPHA_INT br = b_rows - 1; br >= 0; br--)
        {
            ...
            for(ALPHA_INT col = bs - 1; col >= 0; col--)
            {
                ...
                for(ALPHA_INT row = col - 1; row >= 0; row--){
                    ...
                    // temp[row] += A->values[ diagBlock * bs2 +  col * bs + row] * y[y0_offset + br * bs + col ];
                }
            }
        }
        alpha_free(temp);
