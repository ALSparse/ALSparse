                                  ...
                                  ALPHA_Number *diag)
{
    ...
    for (ALPHA_INT cc = global_id; cc < nrhs; cc += stride) {
        ...
        for (ALPHA_INT r = m - 1; r >= 0; r--) {
            ...
            alpha_sube(t, temp);
            alpha_div(y[index2(cc, r, ldb)], t, diag[r]);
...
#undef CSRSM_DIM_Y
    ...
    hipLaunchKernelGGL((trsm_opt_upper<blockdim>), dim3(griddim), dim3(blockdim), 0, handle->stream, csr_row_ptr, csr_col_ind, csr_val, m, nrhs, nnz, alpha, diag, Bt, ldimB, get_value);
