                                  ...
                                  ALPHA_INT ldb_)
{
    ...
    for (ALPHA_INT cc = global_id; cc < nrhs; cc += stride) {
        for (ALPHA_INT r = m - 1; r >= 0; r--) {
            ...
            alpha_sub(y[index2(r, cc, ldb)], t, temp);
            // y[r] = (alpha * x[r] - temp);
        }
    }
}
...

{
    ...
    for (ALPHA_INT j = row_end - 1; j >= row_begin; --j) {
        ...
        // if(cidx == 4999)
        //     printf("tid %u k %d indx %d round %d\n", hipThreadIdx_x, k, scsr_col_ind[k], row_end - j);
      ...
      void *temp_buffer)
{
    ...
    //                    m, nrhs, nnz, alpha, csr_val, csr_row_ptr, csr_col_ind, B, ldb);
    ...
    hipLaunchKernelGGL((trsm_opt_upper<blockdim>), dim3(griddim), dim3(blockdim), 0, handle->stream, csr_row_ptr, csr_col_ind, csr_val, m, nrhs, nnz, alpha, B, ldb, get_value);
