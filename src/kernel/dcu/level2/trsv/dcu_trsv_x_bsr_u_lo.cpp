                ...
                for (ALPHA_INT blkr = 0; blkr < bs; blkr++) {
                    for (ALPHA_INT blkc = 0; blkc <= blkr; blkc++) {
                        if (blkc == blkr) // process diagonals in block
                        {
                            ...
                            alpha_sub(yi, tmp, left_sum[i * bs + blkr]);
      ...
      void *temp_buffer)
{
    ...
    hipLaunchKernelGGL(spts_syncfree_cuda_executor_bsr_wrt_thread, dim3(block_num), dim3(block_size), 0, handle->stream, dir, bsr_row_ptr, bsr_col_ind, bsr_val, bsr_dim, get_value, mb, alpha, x, y, temp);
    hipFree(get_value);
    hipFree(temp);
