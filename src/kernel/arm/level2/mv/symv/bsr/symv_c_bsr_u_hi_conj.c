		              ...
		              ALPHA_Number *y)
{
	...
	ALPHA_Number *part = (ALPHA_Number*)malloc(A->rows * A->block_size * sizeof(ALPHA_Number));
	memset(part, 0, A->rows * A->block_size * sizeof(ALPHA_Number));
	...
	if (A->block_layout == ALPHA_SPARSE_LAYOUT_ROW_MAJOR)
	{
		...
		for(ALPHA_INT k = 0; k < A->rows * A->block_size; k++){
