...
trmv_csc_u_lo_trans_unroll4(const ALPHA_Number alpha,
...
static alphasparse_status_t 
trmv_csc_u_lo_trans_omp(const ALPHA_Number alpha,
                const ALPHA_SPMAT_CSC* A,
                const ALPHA_Number* x,
                const ALPHA_Number beta,
                ALPHA_Number* y)                                                         
...
#pragma omp parallel num_threads(num_threads)
#endif
    {
        ...
        trmv_csc_u_lo_trans_unroll4(alpha,A,x,beta,y,local_n_s,local_n_e);
    }
    return ALPHA_SPARSE_STATUS_SUCCESS;
}
...
ONAME(const ALPHA_Number alpha,
							...
							ALPHA_Number *y)
{
	return trmv_csc_u_lo_trans_omp(alpha, A, x, beta, y); 
}
