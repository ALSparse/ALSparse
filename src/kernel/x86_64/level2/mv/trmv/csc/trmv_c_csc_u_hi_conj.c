    ...
    for (ALPHA_INT i = lrs; i < lre; i++)
    {
        ...
        for (pi = 0; pi < pkl4; pi += 4)
        {
            ...
            if (row_ind3 < i){
                ...
                alpha_madde(tmp1, conj0, x[row_ind0]);
                alpha_madde(tmp2, conj1, x[row_ind1]);
                alpha_madde(tmp3, conj2, x[row_ind2]);
            }else if (row_ind1 < i){
                alpha_madde(tmp2, conj0, x[row_ind0]);
                alpha_madde(tmp3, conj1, x[row_ind1]);
            }else if (row_ind0 < i){
                alpha_madde(tmp3, conj0, x[row_ind0]);
            }
...
static alphasparse_status_t 
trmv_csc_u_hi_conj_omp(const ALPHA_Number alpha,
                const ALPHA_SPMAT_CSC* A,
                const ALPHA_Number* x,
                const ALPHA_Number beta,
                ALPHA_Number* y)                                                         
...
#pragma omp parallel num_threads(num_threads)
#endif
    {
        ...
        trmv_csc_u_hi_conj_unroll4(alpha,A,x,beta,y,local_n_s,local_n_e);
    }
    return ALPHA_SPARSE_STATUS_SUCCESS;
}
...
ONAME(const ALPHA_Number alpha,
		                    ...
		                    ALPHA_Number *y)
{
	return trmv_csc_u_hi_conj_omp(alpha, A, x, beta, y);
}
